...
/// Number of seconds where, if there's no connections to a Val - Code server,
...
/// served the latest version of the Val - Code server whenever they load the
/// page. The Val - Code server prefixes all assets and connections it loads with
/// its version string, so existing clients can continue to get served even
/// while new clients get new Val - Code Server versions.
...
/// Gets the release info from the Val - Code path prefix, which is in the
...
impl ConnectionManager {
	...
	/// Gets the StartData for the a version of the Val - Code server, triggering
