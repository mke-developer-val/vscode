...
/*
 * Implements a standalone CLI app that opens Val - Code from a remote terminal.
 ...
 *  - In external terminals for WSL this calls Val - Code on the Windows side.
 *    The Val - Code desktop executable path is passed in env VSCODE_CLIENT_COMMAND.
 */
...
function fatal(message: string, err: any): void {
	console.error('Unable to connect to Val - Code server: ' + message);
	console.error(err);
	process.exit(1);
}
