...
// Make sure to sync Val - Code state here
...
onceDocumentLoaded(() => {
	...
	if (typeof scrollProgress === 'number' && !settings.settings.fragment) {
		doAfterImagesLoaded(() => {
			...
			// Always set scroll of at least 1 to prevent Val - Code's webview code from auto scrolling us
...
document.addEventListener('click', event => {
	...
	while (node) {
		if (node.tagName && node.tagName === 'A' && node.href) {
			...
			// If original link doesn't look like a url, delegate back to Val - Code to resolve
